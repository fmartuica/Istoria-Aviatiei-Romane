...
if (isset($_POST['reg_user'])) {
  ...
  if ($user) { 
    if ($user['username'] === $username) {
      array_push($errors, "Există deja acest nume de utilizator.");
    }

    if ($user['email'] === $email) {
      array_push($errors, "Există deja acest email.");
    }
  }
  ...
  if (count($errors) == 0) {
  	...
  	if (mysqli_num_rows($results) == 1) {
  	  ...
  	  header('location: index1.php');
  		...
  		array_push($errors, "Parola sau numele de utilizator este greșit.");
  	}
  }
}
